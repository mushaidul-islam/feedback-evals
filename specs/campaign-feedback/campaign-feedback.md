...
Submitting feedback sends the text and campaign ID to the API. The backend looks up the campaign and supplies its name to the existing classifier. Category 1 saves the submitted text. Category 2 saves only the rewritten text. Categories 3 and 4 save nothing. All four categories produce the same visible “sent” result for the submitter. Invalid input, an unknown campaign, or a classification failure must not report a successful submission. Creator pages list only saved feedback, associated with its campaign.
...
Use the frontend's existing NeoBrutalism Base UI components and conventions. The collection page should show a clear submission state without revealing the classifier category or rewrite.
